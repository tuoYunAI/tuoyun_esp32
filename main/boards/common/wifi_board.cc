...
void WifiBoard::StartWifiConfigMode() {
...
#if CONFIG_USE_ESP_UNIFIED_BLE_WIFI_PROVISIONING
    ...
    Application::GetInstance().Schedule([]() {
        ...
        hint += "\n\n";
        Application::GetInstance().Alert(Lang::Strings::WIFI_CONFIG_MODE, hint.c_str(), "", Lang::Sounds::OGG_WIFICONFIG);
    });
#endif
}
...
std::string WifiBoard::GetBoardJson() {
    auto& wifi = WifiManager::GetInstance();
    std::string json = R"({"type":")" + std::string(CONFIG_MANUFACTURER_BOARD_TYPE) + R"(",)";
